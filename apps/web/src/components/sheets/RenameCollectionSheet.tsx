...
interface RenameCollectionSheetProps {
  visible: boolean;
  onClose: () => void;
...
export const RenameCollectionSheet: React.FC<RenameCollectionSheetProps> = ({ visible, onClose, collection, onRename }) => {
  ...
  const handleSave = () => {
    if (!name.trim()) return;
    onRename(name.trim(), selectedEmoji);
    setName('');
    setSelectedEmoji('📁');
  };
    ...
    <div style={{
      ...
      <div style={{
        ...
        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 24 }}>
          <span style={{ fontSize: 16, fontWeight: 600 }}>Rename collection</span>
          <button
            onClick={onClose}
            style={{
              background: 'none',
